...
with st.sidebar:
    ...
    cnpj = "".join(filter(str.isdigit, cnpj_raw))  # Garante CNPJ limpo
    if len(cnpj) != 14:
...
# Preparação de diretórios e objetos de controle
...
# Botão de processamento completo com condicional
...
if st.button("🚀 Processamento Completo", type="primary", disabled=not habilita_botao):
    try:
        if usar_antigo == "Sim" and ult:
            ...
            if faltando:
                lista = "\n- ".join(faltando)
                msg = f"❌ Não foi possível reutilizar: os seguintes arquivos estão ausentes em {retorno_antigo}:\n- {lista}"
                st.error(msg)
            else:
